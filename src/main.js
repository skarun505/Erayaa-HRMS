...
async function bootstrap() {
  try {
    ...
    try {
      ...
      console.error('Seed data error (non-fatal):', seedError);
    }

    // 2. Load User Session
    if (authService.isAuthenticated()) {
      currentUser = authService.getCurrentUser();
      console.log('User authenticated:', currentUser ? currentUser.name : 'Unknown');

      if (!currentUser) {
        throw new Error('Authenticated but user data missing');
      }

      const path = window.location.pathname;
      if (path === '/login') window.history.pushState({}, '', '/');
    ...
    // Auto-recover by clearing session if it's a user data issue
    if (error.message.includes('user') || error.message.includes('Authenticated')) {
      console.warn('Clearing invalid session and reloading...');
      await authService.logout();
      window.location.reload();
      return;
    }
    throw error; // Re-throw to be caught by window.onerror
  }
}
